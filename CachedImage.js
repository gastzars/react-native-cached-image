...
const maxWidth = (d.width / 100) * 60;
const maxHeight = (d.height / 100) * 60;
...
class CachedImage extends React.Component {
  ...
  processSource(source) {
    ...
    imageCacheManager
      ...
      .then(cachedImagePath => {
        RNFS.readFile(cachedImagePath, 'base64')
          .then(fileContents => {
            const dataUri = `data:image/jpeg;base64,${fileContents}`;
            Image.getSize(dataUri, (width, height) => {
              let shouldWidth = width;
              if (shouldWidth > maxWidth) {
                shouldWidth = maxWidth;
              }
              let shouldHeight = (shouldWidth * height) / width;
              if (shouldHeight > maxHeight) {
                shouldHeight = (maxHeight * shouldWidth) / shouldHeight;
              }
              this.safeSetState({
                shouldWidth,
                shouldHeight,
  ...
  render() {
    ...
    const style =
      [
        this.props.style,
        {width: this.state.shouldWidth, height: this.state.shouldHeight},
      ] || styles.image;
    const source =
